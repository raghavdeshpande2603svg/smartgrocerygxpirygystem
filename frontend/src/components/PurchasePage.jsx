...
export default function PurchasePage({ onBack }) {
  ...
  if (!authenticated) {
    return (
      <main className="min-h-screen bg-[#102a2a] px-6 py-10 text-slate-100">
        <div className="mx-auto flex min-h-[80vh] max-w-md items-center">
          <form onSubmit={handleLogin} className="w-full border border-teal-800 bg-[#173737] p-8 shadow-2xl shadow-teal-950/40">
            <button type="button" onClick={onBack} className="mb-12 text-sm text-teal-300 hover:text-white">← Back to dashboard</button>
            <p className="text-xs font-bold uppercase tracking-[0.3em] text-amber-300">Purchase control</p>
            <h1 className="mt-3 text-4xl font-semibold tracking-tight">Vendor ledger</h1>
  ...
  return (
    <main className="min-h-screen bg-[#f3f6f1] px-4 py-5 text-slate-800 sm:px-6 lg:px-10">
      <div className="mx-auto max-w-7xl">
        <header className="flex flex-wrap items-end justify-between gap-4 border-b border-slate-300 pb-6">
          <div><button type="button" onClick={onBack} className="mb-5 text-sm font-semibold text-teal-700 hover:text-teal-900">← Dashboard</button><p className="text-xs font-bold uppercase tracking-[0.28em] text-teal-700">Procurement workspace</p><h1 className="mt-2 text-4xl font-semibold tracking-tight">Purchase</h1><p className="mt-2 text-slate-500">A clear view of who you buy from and where the money goes.</p></div>
          <button type="button" onClick={() => setAuthenticated(false)} className="border border-slate-300 bg-white px-4 py-2 text-sm font-semibold hover:bg-slate-50">Lock section</button>
        </header>
